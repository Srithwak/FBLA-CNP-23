...
function changePoints(key) {
    ...
    let notifsIndex = getIndexFromKey(key, "notifications.json");
    let users = getJSON("user.json");
    console.log(notifs[notifsIndex].name);
    let usersIndex = getIndexFromKey(getKeyFromName(notifs[notifsIndex].name, "user.json"), "user.json");
...
function displayNotifs() {
    ...
    for (let i = 0; i < notifs.length; i++) {
        ...
        let users = getJSON("user.json");
        let usersIndex = getIndexFromKey(getKeyFromName(notifs[i].name, "user.json"), "user.json");
        let admin1 = users[usersIndex].admin;
        let row = `<tr><td><input class ="checked${i}" contenteditable = "true" type = "checkbox" onchange = "verifyChecked(${notifs[i].key}, ${i})" ${x}></td><td>${notifs[i].name}</td><td>${notifs[i].message}</td>`;
        let buttons = `<td>
